...
function InternalGetWindowText(hwnd, buf)
	local ws, sz = WCS(buf or C.GetWindowTextLengthW(hwnd))
	C.InternalGetWindowText(hwnd, ws, sz+1)
	return buf or mbs(ws)
end
...
function FindWindow(hwnd_parent, hwnd_child_after, class_name, window_title)
    return ptr(C.FindWindow(hwnd_parent, hwnd_child_after, class_name, window_title))
end

function ScreenToClient(hwnd, point)
    return C.ScreenToClient(hwnd, point) ~= 0
end
